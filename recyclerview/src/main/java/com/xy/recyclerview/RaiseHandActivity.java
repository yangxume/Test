...
public class RaiseHandActivity extends AppCompatActivity implements RaiseHandAdapter.OnItemClickListener {
...
//        mRecyclerView.addItemDecoration(new DividerItemDecoration(this,DividerItemDecoration.VERTICAL));
    ...
    private void initData() {
        ...
        for (int i = 'A'; i < 'Z'; i++) {
            RaiseHandBean raiseHandBean = new RaiseHandBean("学生"+Integer.toString(i), 0);
            mDatas.add(raiseHandBean);
        }

        mAdapter = new RaiseHandAdapter(this, mDatas);
    ...
    public void onItemClick(View view) {
        ...
        if (DataManager.getDataManager().getSpeakState() == 0){
            ...
            RaiseHandBean raiseHandBean = mAdapter.getDataList().get(position);
            raiseHandBean.state = 1;

            mAdapter.getDataList().set(position, raiseHandBean);

            mAdapter.notifyItemChanged(position,"audio");


        }else{
            Toast.makeText(RaiseHandActivity.this,"连接中,请稍候", Toast.LENGTH_SHORT).show();

 ...
 * @ProjectName: Test
 * @Package: com.xy.t_layout.keywordsflow
 * @ClassName: KeywordsFlow2
 * @Description: java类作用描述
 * @Author: 作者名
 * @CreateDate: 2019-08-01 18:22
 * @UpdateUser: 更新者：
 * @UpdateDate: 2019-08-01 18:22
 * @UpdateRemark: 更新说明：
 * @Version: 1.0
 */
...
public class KeywordsFlow2 extends FrameLayout implements OnGlobalLayoutListener {
    ...
    public KeywordsFlow2(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init();
    }

    public KeywordsFlow2(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public KeywordsFlow2(Context context) {
        super(context);
        init();
    }
    ...
    private boolean show() {
        if (width > 0 && height > 0 && vecKeywords != null
            ...
            for (int i = 0; i < size; i++) {
                ...
                int ranColor = 0xff018fe1;
                // 随机颜色
                // int ranColor = 0xff018fe1 | random.nextInt(0x0077ffff);
                // 随机位置，糙值
                int xy[] = randomXY(random, listX, listY, xItem);
                // 随机字体大小

                int sizeTemp = (int) (TEXT_SIZE_MAX
                        * Math.sqrt((xy[IDX_X] - xCenter)
                        * (xy[IDX_X] - xCenter) + (xy[IDX_Y] - yCenter)
                        * (xy[IDX_Y] - yCenter)) * 2 / width);
                Log.d("lmn", "lmn-sizeTemp = " + sizeTemp);
                if (sizeTemp < TEXT_SIZE_MIN)
                    sizeTemp = TEXT_SIZE_MIN;
                int txtSize = sizeTemp > TEXT_SIZE_MIN ? sizeTemp
                        : TEXT_SIZE_MIN;
                // int txtSize = TEXT_SIZE_MIN + random.nextInt(TEXT_SIZE_MAX -
                // TEXT_SIZE_MIN + 1);
                // 实例化TextView

...
import com.xy.androidsurvey.bean.QuestionModel;
import com.xy.androidsurvey.uitils.UiUtils;
import com.xy.androidsurvey.weight.FlowRadioGroup;

//@SuppressLint("UseSparseArrays")
public class Survey2Activity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		http.send(HttpMethod.GET, url, new RequestCallBack<String>() {
			...
			public void onFailure(HttpException arg0, String arg1) {
				ToastUtils.showLong(Survey2Activity.this, "网络加载失败，使用本地数据加载...");
	...
	private void submitResultBind() {
		...
		tvSubmit.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				// 问题是否回答完整检查
				if (checkResult < 0) {
					ToastUtils.show(Survey2Activity.this, "正在提交");

				} else {
					// 提示未选择的问题
					ToastUtils.show(Survey2Activity.this, "请填写问题" + (checkResult + 1));
				}
			}
		});
			...
			if (questionList.get(i).getQuestionType() == 0) {
				...
				ratingBar.setOnRatingBarChangeListener(new OnRatingBarChangeListener() {
					...
					public void onRatingChanged(RatingBar ratingBar, float rating, boolean fromUser) {
						ToastUtils.show(Survey2Activity.this, "星星：" + ratingBar.getId() + "分数" + rating);
				...
				for (int j = 0; j < questionList.get(i).getQuestionOption().size(); j++) {
					...
					// 设置字体大小
					radio.setTextSize(UiUtils.dip2px(Survey2Activity.this, 5));
					// 设置radio文字
					...
					radio.setOnClickListener(new OnClickListener() {
						@Override
						public void onClick(View v) {
							ToastUtils.show(Survey2Activity.this, "Group" + String.valueOf(group.getId()) + "&Radio：" + radio.getText().toString());
